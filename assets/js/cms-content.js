...
async function init(){try{const [hero,rooms,fac,attr,pol,faq,site,seo,news,blog]=await Promise.all(['hero','rooms','facilities','attractions','policies','faq','site','seo','news','blog'].map(get));
 ...
 const heroEl=$('.hero'); if(heroEl&&hero.slides?.length){heroEl.querySelectorAll('.slide').forEach(x=>x.remove());hero.slides.forEach((x,i)=>{const d=document.createElement('div');d.className='slide'+(i?'':' active');d.style.backgroundImage=`url("${mediaPath(x.image)}")`;d.setAttribute('role','img');d.setAttribute('aria-label',x.alt||hero.title);heroEl.prepend(d)})}
