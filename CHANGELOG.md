# RC6 Release Guard Hotfix 3 — 2026-07-27

## Fixed

- 接送／叫車改為 Lazy Create；新增訂單時不再自動帶入退房日期或建立空白叫車服務。
- 只有使用者實際輸入叫車日期與叫車內容時，才建立或同步接送／叫車服務。
- 修正加收費用核帳語意：加收費用只增加最新應收，不再重複計入已收淨額。
- 訂單尾款結清、剩餘應收為 NT$0 時，自動將該訂單所有正額待核帳紀錄（包含加收費用）更新為「已核帳」。
